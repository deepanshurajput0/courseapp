...
const plan_id = process.env.PLAN_ID || "plan_MGMBLNspkClJuI"
...
export const paymentVerification = catchAsyncError(async(req,res,next)=>{
  const { razorpay_signature, razorpay_payment_id, razorpay_subscription_id } = req.body
     ...
     if(!isAuthenti) return res.redirect(`${process.env.FRONTEND_URL}/ paymentfailed`)
...
//  await payment.deleteOne();
 ...
 await user.save() 
  res.status(200).json({
    ...
    "Subscription cancelled, No refund initiated as subscription as subscription was cancelled after 7 days "  
  })
})